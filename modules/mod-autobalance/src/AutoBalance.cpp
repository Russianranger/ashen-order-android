        ...
        if (map->GetEntry() && map->GetEntry()->IsDungeon())
        {
            ...
            if (AutoBalanceCheck)
            {
                for (Map::PlayerList::const_iterator itr = pl.begin(); itr != pl.end(); ++itr)
                {
                    if (Player* plr = itr->GetSource())
                    {
                        ChatHandler(plr->GetSession()).SendNotification("|cff4cff00%s|rAccess can be unlocked during non-combat", map->GetMapName());
                        ChatHandler(plr->GetSession()).SendNotification("|cffffffff[%s]|rThe player left during the battle|cff4cff00%s|rInstance elastic lock", player->GetName().c_str(), map->GetMapName());
                    }
                }
            }
            else
