...
public:
    ...
    struct npc_ipp_tbc_t4AI: ScriptedAI
    {
        ...
        bool CanBeSeen(Player const* player) override
        {
            ...
            Player* target = ObjectAccessor::FindConnectedPlayer(player->GetGUID());
            return sIndividualProgression->hasPassedProgression(target, PROGRESSION_TBC_TIER_2); //Dinkle - Changed for ZA/BT flip. This is for having passed ssc/tk
        }
    };
...
public:
    ...
    struct npc_ipp_tbc_t5AI : ScriptedAI
    {
        ...
        bool CanBeSeen(Player const* player) override
        {
            ...
            Player* target = ObjectAccessor::FindConnectedPlayer(player->GetGUID());
            return sIndividualProgression->hasPassedProgression(target, PROGRESSION_TBC_TIER_4); //Dinkle - no change requried. this is a check to see if player is ready for sunwell
        }
    };
...
public:
    ...
    struct npc_ipp_tbc_pre_t4AI: ScriptedAI
    {
        ...
        bool CanBeSeen(Player const* player) override
        {
            ...
            Player* target = ObjectAccessor::FindConnectedPlayer(player->GetGUID());
            return sIndividualProgression->isBeforeProgression(target, PROGRESSION_TBC_TIER_4); //Dinkle - remains the same for ZA/BT flip. Honestly irrelevant now with Badge changes as this was just for G'eras.
        }
    };
