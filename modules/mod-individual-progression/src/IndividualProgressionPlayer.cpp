...
public:
    ...
    bool OnBeforeTeleport(Player* player, uint32 mapid, float x, float y, float z, float /*orientation*/, uint32 /*options*/, Unit* /*target*/) override
    {
        ...
        {
            ChatHandler(player->GetSession()).PSendSysMessage("You must complete Molten Core before entering.");
            return false;
        }
      ...
      //  }
        ...
        {
            ChatHandler(player->GetSession()).PSendSysMessage("You must complete The Scepter of the Shifting Sands questline before entering.");
            return false;
        }
        ...
        if (mapid == MAP_OUTLANDS)
        {
            ...
            {
                ChatHandler(player->GetSession()).PSendSysMessage("You must complete The Black Temple before going to the Isle of Quel'Danas.");
                return false;
            }
        }
        if (mapid == MAP_ZUL_AMAN && !sIndividualProgression->hasPassedProgression(player, PROGRESSION_TBC_TIER_2)) //changed to be after ssc/tk
        {
            ChatHandler(player->GetSession()).PSendSysMessage("You must complete Tempest Keep and Serpentshrine Cavern before entering.");
            return false;
        }
        if (mapid == MAP_NORTHREND && !sIndividualProgression->hasPassedProgression(player, PROGRESSION_TBC_TIER_5))
        {
            ChatHandler(player->GetSession()).PSendSysMessage("You must complete Sunwell Plateau before going to Northrend.");
            return false;
        }
        if (mapid == MAP_ULDUAR && !sIndividualProgression->hasPassedProgression(player, PROGRESSION_WOTLK_TIER_1))
        {
            ChatHandler(player->GetSession()).PSendSysMessage("You must complete Tier 7 content before entering Ulduar.");
            return false;
        }
        if ((mapid == MAP_TRIAL_OF_THE_CHAMPION || mapid == MAP_TRIAL_OF_THE_CRUSADER) && !sIndividualProgression->hasPassedProgression(player, PROGRESSION_WOTLK_TIER_2)){
            ChatHandler(player->GetSession()).PSendSysMessage("You must complete Ulduar before entering.");
            return false;
        }
        ...
        {
            ChatHandler(player->GetSession()).PSendSysMessage("You must complete Trial of the Crusder before entering.");
            return false;
        }
        if (mapid == MAP_RUBY_SANCTUM && !sIndividualProgression->hasPassedProgression(player, PROGRESSION_WOTLK_TIER_4))
        {
            ChatHandler(player->GetSession()).PSendSysMessage("You must complete Icecrown Citadel before entering.");
            return false;
        }
    ...
    bool CanJoinLfg(Player* player, uint8 roles, lfg::LfgDungeonSet& dungeons, const std::string& comment) override
    {
        ...
        {
            ChatHandler(player->GetSession()).SendNotification("The Looking For Group feature is currently disabled by the Individual Progression module.");
