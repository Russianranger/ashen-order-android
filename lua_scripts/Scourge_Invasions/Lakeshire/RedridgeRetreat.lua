...
local targetLocations = {
    {x = -9195.19, y = -2157.19, z = 57.186, o = 1.0},
    {x = -9201.69, y = -2147.54, z = 71.211, o = 1.0},
    {x = -9231.74, y = -2148.622, z = 71.211, o = 1.0}
...
local function MoveNpcsToLocation(event, gameEventId)
    ...
    local targetLocation = targetLocations[math.random(#targetLocations)]
            local randomX = targetLocation.x + math.random(-2, 2)
            local randomY = targetLocation.y + math.random(-2, 2)
            local randomizedOrientation = (targetLocation.o or npc:GetO()) + math.random() * 0.2 - 0.1  -- Randomize orientation
            npc:MoveTo(0, randomX, randomY, targetLocation.z, true)
            npc:SetHomePosition(randomX, randomY, targetLocation.z, randomizedOrientation)
                end
            end
        end
    end
...
local function DespawnNpcs(event, gameEventId)
    if gameEventId == GAME_EVENT_ID then
        ...
        for _, player in ipairs(players) do
            ...
            npc:Respawn()
            npc:SetHomePosition(npc:GetX(), npc:GetY(), npc:GetZ(), npc:GetO()) -- Set home position to respawn point
        end
    end
end
    end
