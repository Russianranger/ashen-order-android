...
function Core.prototype.GetTransmogCost(self, newSkin)
    local skin = self.Data.skinCollectionCache[tonumber(newSkin)]
    if not skin then
        print("Error: skinCollectionCache entry for newSkin is nil. newSkin:", newSkin)
        return 0  -- Return a default cost or handle the error appropriately
    end
    local sellPrice = skin.SellPrice or 0  -- Ensure SellPrice is not nil
    local itemPrice = 0
