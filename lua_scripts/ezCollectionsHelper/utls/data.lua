...
function Data.prototype.ApplyTransmog(self, playerGuid, itemGuid, fakeEntry)
    print("Applying transmog:", playerGuid, itemGuid, fakeEntry)
    ...
    if result then
        print("Transmog applied successfully.")
    else
        print("Failed to apply transmog.")
    end
    return result ~= nil
end

function Data.prototype.DeleteTransmog(self, playerGuid, itemGuid)
    print("Deleting transmog:", playerGuid, itemGuid)
    local result = CharDBQuery(string.format(self.removeTransmogQuery, playerGuid, itemGuid))
    if result then
        print("Transmog deleted successfully.")
    else
        print("Failed to delete transmog.")
    end
    return result ~= nil
end

function Data.prototype.CleanTransmogDb(self, playerGuid)
    print("Cleaning transmog DB for player:", playerGuid)
    __TS__ArrayForEach(
    ...
    if queryResult then
        repeat
            ...
            if self.skinCollectionCache[skinsOwned.RealEntry] ~= nil then
                ...
                print("Error: skinCollectionCache entry for RealEntry is nil. RealEntry:", skinsOwned.RealEntry)
            end
        until not queryResult:NextRow()
    end
    return result
...
function Data.prototype.SearchAppearances(self, query, slot, accountId)
    ...
    for ____, key in ipairs(__TS__ObjectKeys(self.skinCollectionCache)) do
        ...
        if #query == 0 or query == nil then
            ...
            if skinCollection.Slot == slot and string.lower(skinCollection.Name):find(string.lower(query)) then
                result[#result + 1] = skinCollection.Id
            end
        end
        ::__continue45::
...
function Data.generateFlags(self, value)
    local flags = {}
    for i = #Common.RaceData - 1, 0, -1 do
...
function Data.prototype.AddItemToCache(self, itemEntry)
    local query = string.format("SELECT entry, InventoryType, Material, AllowableClass, AllowableRace, name, VerifiedBuild, Quality, SellPrice, class, subclass FROM item_template WHERE entry = %d", itemEntry)
    ...
    if queryResult then
        ...
        if Common.Settings:AllowedQuality(skinCollectionList.Quality) then
            ...
            self.skinCollectionCache[skinCollectionList.Id] = skinCollectionList
            print("Item added to cache: ", skinCollectionList.Id)
        end
    else
        print("Error: Failed to fetch item details for entry: ", itemEntry)
    end
end
return ____exports