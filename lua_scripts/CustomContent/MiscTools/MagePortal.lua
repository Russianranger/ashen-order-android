...
local function MagePortalResponse(playerName, city, spellId, hasRune)
    ...
    if player then
        local objects = player:GetNearObjects(30, 3, 0, 2)
        local mageBotFound = false
        for _, object in ipairs(objects) do
            local creature = object:ToCreature()
            if creature then
                local scriptName = creature:GetScriptName()
                if scriptName == "mage_bot" and not scriptName:match("_pet_bot$") then
                    mageBotFound = true
        ...
        if not mageBotFound then
            player:SendAreaTriggerMessage("No suitable mage bot found nearby!")
        end
    end
end
...
local function MageBotGreetingResponse(playerName, greeting)
    ...
    if player then
        ...
        for _, object in ipairs(objects) do
            ...
            if creature then
                local scriptName = creature:GetScriptName()
                if scriptName:match("_bot$") and not scriptName:match("_pet_bot$") then
                    creature:SetFacingToObject(player)
...
local function PaladinBotDivineIntervention(playerName)
    local player = GetPlayerByName(playerName)
    if player then
        local objects = player:GetNearObjects(60, 3, 0, 2) 
        for _, object in ipairs(objects) do
            local creature = object:ToCreature()
            local scriptName = creature:GetScriptName()
            if creature and scriptName == "paladin_bot" and not scriptName:match("_pet_bot$") then
                creature:SetFacingToObject(player)
...
local function UseSapperCharge(playerName)
    local player = GetPlayerByName(playerName)
    local firstBotResponded = false
    if player then
        if player:GetLevel() < 45 then
            player:SendAreaTriggerMessage("You must be at least level 45 to use sapper charges!")
            return
        end
        local objects = player:GetNearObjects(40, 3, 0, 2)
        for _, object in ipairs(objects) do
            local creature = object:ToCreature()
            if creature then
                local scriptName = creature:GetScriptName()
                if scriptName:match("_bot$") and not scriptName:match("_pet_bot$") then
                    ...
                    if not creature:HasAura(844053) then
                        creature:CastSpell(creature, 13241, true)

                        if not firstBotResponded then
                            creature:SendUnitSay("|cffffffffDeploying sapper charge!|r", 0)
                            firstBotResponded = true
                        end
                    else
                        if not firstBotResponded then
                            local aura = creature:GetAura(844053)
                            if aura then
                                local durationMs = aura:GetDuration()
                                local minutes = math.floor(durationMs / 60000)
                                local seconds = math.floor((durationMs % 60000) / 1000)

                                local timeRemainingMsg = string.format("|cffffffffStill on cooldown! %d minute(s) and %d second(s) until we can use another sapper!|r", minutes, seconds)
                                creature:SendUnitSay(timeRemainingMsg, 0)
                                firstBotResponded = true
                            end
                        end
                    end
        ...
        if not firstBotResponded then
            player:SendAreaTriggerMessage("No suitable bot found nearby!")
        end
    end
end

local function CheckForSapperChargeRequest(msg, player)
    local normalizedMsg = msg:lower()
    if normalizedMsg == "use sapper charges" then
        local playerName = player:GetName()
        player:RegisterEvent(function()
            UseSapperCharge(playerName)
        end, 500, 1)  
    end
end
...
local function MageBotOnPlayerChat(event, player, msg, Type, lang)
    ...
    CheckForSapperChargeRequest(msg, player) 
    ...
    local factionPortals = {}
    if player:IsAlliance() then
        for city, spellId in pairs(alliancePortals) do
            factionPortals[city] = spellId
        end
    else
        for city, spellId in pairs(hordePortals) do
            factionPortals[city] = spellId
        end
    end
