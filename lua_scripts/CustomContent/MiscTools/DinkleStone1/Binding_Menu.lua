...
Version: 1.5.0
Made by: MadBuffoon
Notes: This adds a menu that you can bind to a location and easily return to. 
...
local CooldownAura = 882054 

-- Do not change or remove
local Gold = 10000
local GuidN = 0

local function IsInRestrictedArea(player)
    -- Allow GMs or player named "Faith" to access the menu in restricted areas
    if player:IsGM() or player:GetName() == "Faith" then
        return false
    end

    local map = player:GetMap()
    -- Check if the map is a dungeon, raid, or battleground
    return map:IsRaid() or map:IsDungeon() or map:IsBattleground()
end
...
--(Start) Pulls for the guid for the player
...
function BindMenuGossip(event, player)
    if player:IsInCombat() then
        player:SendBroadcastMessage("You cannot bind while in combat.")
        return
    elseif IsInRestrictedArea(player) then
        player:SendBroadcastMessage("You cannot bind in dungeons, raids, or battlegrounds, or unless your name is Faith.")
        return
	elseif player:HasAura(CooldownAura) then
		local aura = player:GetAura(CooldownAura)
		local remainingTime = aura:GetDuration() / 1000 -- Remaining time in seconds
		local minutes = math.floor(remainingTime / 60)
		local seconds = remainingTime % 60
		player:SendBroadcastMessage(string.format("You must wait %d minutes and %d seconds before teleporting via the bind menu again.", minutes, seconds))
...
local PlayerName = player:GetName()
    ...
    --New code to check the number of bindings for a character due to gossip menu limit
        ...
        repeat
            ...
            if(intid == entryNumber_Use) then
                ...
                if query3 then
                    ...
                    player:AddAura(CooldownAura, player) -- Add the cooldown aura after teleporting
                end
                player:GossipComplete()
            end
