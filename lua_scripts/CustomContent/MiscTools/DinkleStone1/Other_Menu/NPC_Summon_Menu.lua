...
local function OnSelect(event, player, _, sender, intid, code)
	...
	for k, v in pairs(NPC_Spawn) do
		...
		if(intid == v[1] + 100) then
			-- Check if the transmogifier is being summoned
			if v[1] == 190010 then
				player:CastSpell(player, 2000100) -- Cast the desired spell
				player:GossipComplete()
				return
			-- Handle other NPCs in your list
			elseif v[1] == 24939 then
				...
				if NPC_Trainers[playerClass] then
					...
					local TrainerNPC_Spawned
						if TrainerNPC == nil or NPC_CheckNear == false then
							TrainerNPC_Spawned = player:SpawnCreature( Trainer, x+1, y+1, z+0.5, o-3.5, 1, 120000 )
			...
			if NPC_Spawned == nil or NPC_CheckNear == false then
				NPC = player:SpawnCreature( v[1], x+1, y+1, z+0.5, o-3.5, 1, 120000 )
