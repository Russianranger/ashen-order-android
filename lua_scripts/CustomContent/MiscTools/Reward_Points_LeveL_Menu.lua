...
CLASS_SPELLS = {
    [CLASS_IDS.WARRIOR] = {100, 772, 3127, 6343, 34428, 284, 1715, 2687, 6546, 355, 7386, 72,
    5242, 7384, 1160, 6572, 285, 694, 2565, 676, 8198, 845, 6547, 12678,
    20230, 5246, 6192, 1608, 5308, 6190, 6574, 1161, 6178, 871, 8204, 1464, 6548,
    7369, 20252, 11549, 11564, 18499, 20658, 7379, 11554, 1680, 6552,
    ...
    47436, 47502, 47437, 47475, 47440, 47471, 47488, 57755, 57823, 846924, 46968},
    [CLASS_IDS.PALADIN] = {853, 1152, 3127, 19740, 20271, 498, 639, 633, 1022, 10290, 19834, 53408,
	647, 19742, 31789, 7294, 25780, 62124, 1044, 643, 879, 5502, 19750,
    20217, 26573, 1026, 19746, 19835, 20164, 5588, 5599, 10326, 19850,
    1038, 10298, 19939, 5614, 19876, 53407, 1042, 2800, 10291, 19752, 20116,
    20165, 19836, 19888, 642, 19852, 19940, 5615, 10299, 19891, 3472,
    10278, 20166, 750, 1032, 5589, 19895, 20922, 4987, 19837, 19941, 10312,
    19853, 19897, 24275, 6940, 10300, 10328, 19899, 2812, 10292, 10310,
    19942, 20923, 10313, 19838, 19896, 24274, 25782, 10308, 10329, 19854, 25894,
    10301, 19898, 19943, 10293, 10314, 10318, 19900, 20924, 24239, 25290,
    ...
    27173, 31884, 48935, 48937, 54428, 48816, 48800, 48931, 48933, 48784,
    48805, 48941, 48781, 48818, 53600, 48943, 54043, 48936, 48938, 48945, 48788,
    48817, 48947, 48785, 48801, 48932, 48934, 48942, 48782, 48806, 48819,
    53601, 61411},
    [CLASS_IDS.HUNTER] = {1978, 13163, 1130, 3044, 3127, 5116, 14260, 13165, 13549, 19883, 1515,
    ...
    [CLASS_IDS.ROGUE] = {53, 921, 1757, 1776, 5277, 6760, 2983, 5171, 6770, 1766, 2589, 3127,
    ...
    [CLASS_IDS.PRIEST] = {589, 2052, 17, 591, 139, 586, 594, 2006, 2053, 8092, 588, 592, 1244, 528,
    ...
    [CLASS_IDS.SHAMAN] = {8042, 8071, 332, 2484, 324, 529, 5730, 8018, 8044, 8024, 8050, 8075, 3599,
    ...
    [CLASS_IDS.MAGE] = {116, 5504, 143, 587, 2136, 118, 205, 5143, 122, 5505, 7300, 130, 145, 597, 604,
    ...
    [CLASS_IDS.WARLOCK] = {348, 172, 702, 695, 1454, 980, 5782, 696, 707, 1120, 6201, 697, 705, 755, 1108,
    ...
    [CLASS_IDS.DRUID] = {774, 8921, 467, 5177, 339, 5186, 99, 1058, 5232, 8924, 16689, 6795,
    6807, 18960, 5229, 8936, 50769, 782, 5178, 5187, 5211, 8946, 779, 
    1430, 8925, 770, 1062, 2637, 6808, 8938, 16810, 16857, 1079, 1082, 1735,
    ...
    8910, 8918, 8929, 9000, 16914, 20719, 20742, 22827, 29166, 62600, 6787,
...
function RemoveSpellsForClass(player)
    local classSpellsToRemove = CLASS_SPELLS[player:GetClass()]
...
function RedeemMenuGossip(event, player)
	...
	if query then
		...
		player:GossipMenuAddItem(4, "|TInterface\\Icons\\Spell_chargepositive:45:45:-40|t Redeem Levels for Reward Points.", 0, 10, false, "Reward Points\nThis will make you level 1 again, remove all current bots, trained spells, and most of your items will be sent to the account wide gear depository for later pickup at an appropriate level.\nAre you sure?")
...
local function EscapeSQLString(str)
    -- Replace apostrophes with two apostrophes as a simple form of escaping
    return string.gsub(str, "'", "''")
end

local function HandleItemDepositAndRemoval(player)
    local playerAccount = player:GetAccountId()
    local itemsToSkip = {"Hearthstone", "Dinklestone", "Reward Menu"}  -- List of item names to skip

    -- Function to check if the item should be skipped
    local function shouldSkipItem(itemName, inventoryType)
        for _, skipItemName in ipairs(itemsToSkip) do
            if itemName == skipItemName then
                return true
            end
        end

        -- Check if the inventoryType corresponds to equipment or bags and skip accordingly
        if inventoryType == 0 or inventoryType == 4 or inventoryType == 19 then
            return true
        end
    ...
    local function processItem(bag, slot)
        ...
        if item then
            ...
            local inventoryType = item:GetInventoryType() -- Get the item's inventory type

            -- Skip the item if its name or inventoryType is in the itemsToSkip list
            if shouldSkipItem(itemName, inventoryType) then
                return
            end
            ...
            -- Insert into account_item_deposit
            local escapedItemName = EscapeSQLString(itemName)  -- Make sure to escape the item name
            local itemlevel = item:GetItemLevel()  -- Get item level
            local depositSQL = string.format("INSERT IGNORE INTO account_item_deposit (account_id, item_entry, item_name, itemlevel, requiredlevel) VALUES (%i, %i, '%s', %i, %i)", playerAccount, itemEntry, escapedItemName, itemlevel, requiredLevel)
            CharDBExecute(depositSQL)

            -- Remove the item from the player's inventory
            player:RemoveItem(itemEntry, item:GetCount())
        end
    end

    -- Equipment and Backpack
    for slot = 0, 38 do -- Equipment, equipped bags, and backpack
        processItem(255, slot)
    end

    -- Bank Main Slots and Bank Bag Slots
    for slot = 39, 74 do -- Bank main slots and bank bag slots
        processItem(255, slot)
    end

    -- Keyring
    for slot = 86, 117 do -- Keyring
        processItem(255, slot)
    end
...
local function OnSelect(event, player, _, sender, intid, code)
	...
	local playerLeveL = player:GetLevel()
	local leftToMax = maxLevels - playerLeveL
	local playerGUID = player:GetGUIDLow() -- Get the player's GUID
    ...
    if(intid == 1 or intid == 10) then -- Deposit or Redeemed Reward Points
        -- SQL to delete quest data
        local deleteQuestStatusSQL = string.format("DELETE FROM character_queststatus WHERE guid=%i", playerGUID)
        local deleteQuestStatusRewardedSQL = string.format("DELETE FROM character_queststatus_rewarded WHERE guid=%i", playerGUID)

        -- Execute the SQL commands
        CharDBExecute(deleteQuestStatusSQL)
        CharDBExecute(deleteQuestStatusRewardedSQL)
        HandleItemDepositAndRemoval(player)
        ...
        if intid == 1 then -- Deposit
            ...
            kickPlayer(player,5)
        elseif intid == 10 then -- Redeemed Reward Points
