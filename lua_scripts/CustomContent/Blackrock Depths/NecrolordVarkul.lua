...
function NecrolordVarkul.CastShadowBolt(event, delay, pCall, creature)
    creature:CastSpell(creature:GetVictim(), NecrolordVarkul.SPELL_IDS.SHADOW_BOLT, true)
end
...
function NecrolordVarkul.CastBlink(event, delay, pCall, creature)
    ...
    if math.random(1, 2) == 1 then
        creature:RegisterEvent(NecrolordVarkul.CastBlink, 1000, 1)
    end
end

function NecrolordVarkul.CastShadowflame(event, delay, pCall, creature)
    if math.random(1, 4) <= 3 then 
        creature:CastSpell(creature:GetVictim(), NecrolordVarkul.SPELL_IDS.SHADOWFLAME, false)
    end
end
...
function NecrolordVarkul.EnterCombat(event, creature, target)
	...
	creature:RegisterEvent(NecrolordVarkul.CastShadowflame, 16000, 0)
end
