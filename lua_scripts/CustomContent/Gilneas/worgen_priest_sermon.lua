...
local function StartSermon(npc)
    if not npc:GetData("sermonStarted") then
        npc:SetData("sermonStarted", true)
        npc:RegisterEvent(SendSermonText, 3000, 1)
    end
end

local function OnCreatureMoveInLOS(event, creature, unit)
    if creature:GetEntry() == NPCID and unit:IsPlayer() then
        StartSermon(creature)
    end
end

RegisterCreatureEvent(NPCID, 27, OnCreatureMoveInLOS)