...
local function Smolder_OnEnterCombat(event, creature, target)
	...
	creature:RegisterEvent(CastSmolderBomb, 13000, 0)
    creature:RegisterEvent(CastScorch, 1100, 0)
--    creature:RegisterEvent(CastFlameBreath, 20500, 0)
    creature:RegisterEvent(CastCharredEarth, 15000, 0)
    creature:RegisterEvent(CastPyroblast, 24500, 0)
    creature:RegisterEvent(CastSummonElemental, 60000, 0)
	creature:RegisterEvent(CastTailSweep, 10000, 0)
...
function Smolder.DespawnCreatures(creature) -- Despawns any flame tsunamis. I set this for on death and on leave combat
    local creatureEntryList = {83006, 83007, 83008, 83009, 83010, 83011, 83012, 83013, 83014, 83015, 83016, 83017, 83018, 83019, 83020, 387570, 12265, 11666}
...
local function Smolder_OnLeaveCombat(event, creature)
    Smolder.DespawnCreatures(creature) -- despawns Tsunamis and other stuff
	creature:CastSpell(self, 875167, true)
    creature:RemoveEvents()
end

local function Smolder_OnDied(event, creature, killer)
    creature:CastSpell(self, 875167, true)
    creature:SendUnitYell("My fire...extinguished...", 0)
    creature:PlayDirectSound(183259)
    Smolder.DespawnCreatures(creature)
    creature:RemoveEvents()
	local target = creature:GetNearestCreature(350, 800066)  
    if target then
...
--local function Smolder_OnDamageTaken(event, creature, attacker, damage)
...
local function Smolder_OnSpawn(event, creature)
...
RegisterCreatureEvent(83001, 1, Smolder_OnEnterCombat)
RegisterCreatureEvent(83001, 2, Smolder_OnLeaveCombat)
RegisterCreatureEvent(83001, 4, Smolder_OnDied)
--RegisterCreatureEvent(83001, 9, Smolder_OnDamageTaken)
RegisterCreatureEvent(83001, 5, Smolder_OnSpawn)
