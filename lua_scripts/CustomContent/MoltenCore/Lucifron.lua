...
function Lucifron.CastImpendingDoom(eventId, delay, calls, creature)
    ...
    creature:CastSpell(randomTarget, 19702, true)
end
...
function Lucifron.CastShadowShock(eventId, delay, calls, creature)
    creature:CastSpell(creature:GetVictim(), 19460, true)
end
...
local function RespawnCreatures(creature, npcId1, npcId2, range)
    print("Respawning creatures within range: " .. range)

    local creatures = creature:GetCreaturesInRange(range, npcId1, 0, 0) 
    if #creatures == 0 then
        print("No creatures found with NPC ID: " .. npcId1)
    else
        for _, c in ipairs(creatures) do
            if c:IsDead() then
                c:Respawn()
                print("Respawned creature with NPC ID: " .. npcId1)
            end
        end
    end
    ...
    if #creatures == 0 then
        print("No creatures found with NPC ID: " .. npcId2)
    else
        for _, c in ipairs(creatures) do
            if c:IsDead() then
                c:Respawn()
                print("Respawned creature with NPC ID: " .. npcId2)
            end
        end
    end
end
...
function Lucifron.OnLeaveCombat(event, creature)
    print("Lucifron left combat.")
    creature:RemoveEvents()
	creature:CastSpell(self, 875167, true)

    if creature:IsAlive() then
        print("Lucifron is alive, checking for creatures to respawn.")
        RespawnCreatures(creature, 11662, 12119, 150)
    else
        print("Lucifron is dead, not respawning creatures.")
    end
end
...
function Lucifron.OnDamageTaken(event, creature, attacker, damage)
    if not creature:HasAura(38166) and creature:HealthBelowPct(10) then
