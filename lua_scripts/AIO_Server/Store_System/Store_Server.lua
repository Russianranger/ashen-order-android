...
local CONFIG = {
	...
	mailSenderGUID = 4, -- GUID of the character shown as sender of purchase mails
	strings = {
...
function StoreHandler.UpdateCurrencies(player)
	...
	for currencyId, currency in pairs(GetCurrencyData()) do
		...
		if(currencyTypeText == "HONOR_POINTS") then
            val = player:GetHonorPoints(currency[KEYS.currency.data])
        end
        
        if(currencyTypeText == "ARENA_POINTS") then
            val = player:GetArenaPoints(currency[KEYS.currency.data])
        end
...
function SHOP_UI.DeductCurrency(player, currencyId, amount)
	...
	-- Other special handling you have to add your own integration here.
	if(CURRENCY_TYPES[currencyType] == "SERVER_HANDLED") then
		return false
	end

	-- Honor Points handling
	if(CURRENCY_TYPES[currencyType] == "HONOR_POINTS") then
        if (amount > player:GetHonorPoints()) then
            player:SendAreaTriggerMessage("|cFFFF0000"..CONFIG.strings.insufficientFunds.." "..currencyName.."|r")
            player:SendBroadcastMessage("|cFFFF0000"..CONFIG.strings.insufficientFunds.." "..currencyName.."|r")
            player:PlayDirectSound(GetSoundEffect("notEnoughMoney", player:GetRace(), player:GetGender()), player)
            return false
        end
        
        player:SetHonorPoints(player:GetHonorPoints() - amount) 
    end

    -- Arena Points handling
	if(CURRENCY_TYPES[currencyType] == "ARENA_POINTS") then
        if (amount > player:GetArenaPoints()) then
            player:SendAreaTriggerMessage("|cFFFF0000"..CONFIG.strings.insufficientFunds.." "..currencyName.."|r")
            player:SendBroadcastMessage("|cFFFF0000"..CONFIG.strings.insufficientFunds.." "..currencyName.."|r")
            player:PlayDirectSound(GetSoundEffect("notEnoughMoney", player:GetRace(), player:GetGender()), player)
            return false
        ...
        player:SetArenaPoints(player:GetArenaPoints() - amount) 
    end
