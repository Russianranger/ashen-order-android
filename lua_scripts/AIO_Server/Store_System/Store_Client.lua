...
function SHOP_UI.MainFrame_Create()
	...
	shopFrame:SetPoint("TOPLEFT", 20, -80)
	shopFrame:Hide()
	    -- Enable mouse wheel event on the frame
    shopFrame:EnableMouseWheel(true)
    shopFrame:SetScript("OnMouseWheel", function(self, delta)
        if delta > 0 then
            SHOP_UI.PageButtons_OnClick(-1)  -- Scroll up, previous page
        else
            SHOP_UI.PageButtons_OnClick(1)   -- Scroll down, next page
        end
    end)
...
function SHOP_UI.ServiceBoxes_Create(parent)
	...
	for i = 1, 8 do
		...
		service.PriceFont:SetPoint("CENTER", service, "CENTER", -5, -55)
		
		-- price currency icon
		service.currencyIcon = service:CreateTexture(nil, "OVERLAY")
		service.currencyIcon:SetSize(15, 15)
		service.currencyIcon:SetPoint("LEFT", service.PriceFont, "RIGHT", 3, 0)
		...
		service.DicountFont:SetPoint("CENTER", service.PriceFont, "CENTER", 0, 14)
		...
		service.Banner.Background:SetSize(88, 23)
		...
		service.BannerText:SetPoint("CENTER", service.Banner.Background, 0, 1)
		...
		service.buyButton:SetSize(100, 25)
		...
		service.buyButton.ButtonText:SetText("Purchase")
...
function SHOP_UI.ModelFrame_Create(parent)
	...
	modelFrame.CloseButton:SetPoint("TOPRIGHT", modelFrame, "TOPRIGHT", 0, 0)
	...
	modelFrame.playerModel:SetPoint("CENTER", modelFrame, "CENTER", 20, 0)
	modelFrame.playerModel:SetSize(430, 480)

	-- Enable player model frame mouse dragging
	local turnSpeed = 34
	local dragSpeed = 100
	local zoomSpeed = 0.5
	...
	modelFrame.creatureModel:SetSize(455, 440)

	-- Enable creature model frame mouse dragging
	local turnSpeed = 34
	local dragSpeed = 100
	local zoomSpeed = 0.5
	...
	modelFrame.leftButton:SetPoint("TOPLEFT", modelFrame, "TOPLEFT", 7, -40)
	...
	modelFrame.rightButton:SetPoint("TOPLEFT", modelFrame, "TOPLEFT", 37, -40)
...
function SHOP_UI.ModelFrame_ShowPlayer(displayId)
	...
	if(displayId) then
		SHOP_UI["MODEL_FRAME"].playerModel:Undress()
...
local function ModifyGameMenuFrame()
	...
	frame:SetSize(185, 328)
	
	-- move the buttons down
	local videoButton = _G["GameMenuButtonOptions"]
	videoButton:SetPoint("CENTER", frame, "TOP", 0, -96)


	--[[
	    Aldori note 3/11/2024:

		If you're using ElvUI and want to use a skinned version of the store button, follow these instructions:
		
			- Comment out the below lines (1237-1252).
			- Copy/paste those lines into ElvUI Init.lua (lines 176)

		The issue was the button wasn't getting skinned because ElvUI was skinning the buttons before this button was event created.
		So with moving this code into ElvUI Init.lua, the button gets created with ElvUI.
	]]--
	
	-- add store button to the game menu
	local storeButton = CreateFrame("Button", "StoreButton", frame, "UIPanelButtonTemplate");
	storeButton:SetPoint("CENTER", frame, 0, 124)
	storeButton:SetSize(150, 25)
