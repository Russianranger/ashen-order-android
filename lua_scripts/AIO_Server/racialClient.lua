...
button:SetScript("OnClick", function(self)
    ...
    if maxActive < maximumActive or (isSpecialSpell and specialSpellActive and not self.active) then
        if not self.active then
            ...
            if isSpecialSpell then
                for _, spellId in ipairs(specialSpellsGroup) do
                    if spellId ~= id then
                        AIO.Handle("RACIAL_SERVER", "racialActivate", spellId, itemType, true)
                    end
                end
            end
        else
...
local function resetSpells()
    -- List of profession spell IDs to skip
    ...
    for tabIndex, spellList in ipairs(customRacial.racialSpells) do
        for _, spellInfo in ipairs(spellList) do
            -- Skip resetting the spell if it is a profession spell
            if not professionSpells[spellInfo.id] then
                -- Check if the spell is known or if the item exists
                if spellInfo.itemType == "spell" and IsSpellKnown(spellInfo.id) then
    ...
    -- Reset active racial spells and update UI
    customRacial.activeRacialSpells = {}
    activeRacialSpells()  -- Call this function to update the UI with the new active racial spell count
end
