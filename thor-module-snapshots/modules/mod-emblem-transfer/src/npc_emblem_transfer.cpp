...
enum Actions
{
    ...
    ACTION_TRANSFER_JUSTICE = 1007,
    ACTION_TRANSFER_GLORY = 1008,
    ACTION_TRANSFER_COURAGE = 1009,
    ACTION_CLOSE = 1010
};
...
enum Items
{
    ...
    ITEM_BADGE_OF_GLORY = 829434,
    ITEM_BADGE_OF_COURAGE = 829435
};
...
enum SenderMenu
{
    ...
    GOSSIP_SENDER_TRANSFER_JUSTICE = 1006,
    GOSSIP_SENDER_TRANSFER_GLORY = 1007,
    GOSSIP_SENDER_TRANSFER_COURAGE = 1008
};
...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
        ...
        if (sConfigMgr->GetOption<bool>("EmblemTransfer.allowBadgesJustice", false))
            AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "Transfer my Badges of Justice", GOSSIP_SENDER_MAIN, ACTION_TRANSFER_JUSTICE);

        if (sConfigMgr->GetOption<bool>("EmblemTransfer.allowBadgesGlory", false))
            AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "Transfer my Badges of Glory", GOSSIP_SENDER_MAIN, ACTION_TRANSFER_GLORY);

        if (sConfigMgr->GetOption<bool>("EmblemTransfer.allowBadgesCourage", false))
            AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "Transfer my Badges of Courage", GOSSIP_SENDER_MAIN, ACTION_TRANSFER_COURAGE);
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 sender, uint32 action)
    {
        ...
        if (action == ACTION_RETRIEVE_EMBLEMS)
        {
            ...
            if (result)
            {
                do
                {
                    ...
                    {
                        ChatHandler(player->GetSession()).PSendSysMessage(LANG_ITEM_CANNOT_CREATE, emblemId, noSpaceForCount);
                        continue;
                    }
        ...
        if (sender == GOSSIP_SENDER_MAIN)
        {
            ...
            case ACTION_TRANSFER_JUSTICE:
                newSender = GOSSIP_SENDER_TRANSFER_JUSTICE;
                emblems = player->GetItemCount(ITEM_BADGE_OF_JUSTICE);
                break;

            case ACTION_TRANSFER_GLORY:
                newSender = GOSSIP_SENDER_TRANSFER_GLORY;
                emblems = player->GetItemCount(ITEM_BADGE_OF_GLORY);
                break;

            case ACTION_TRANSFER_COURAGE:
                newSender = GOSSIP_SENDER_TRANSFER_COURAGE;
                emblems = player->GetItemCount(ITEM_BADGE_OF_COURAGE);
                break;
            }

            if (emblems < minAmount)
            {
                ChatHandler(player->GetSession()).PSendSysMessage("You don't have enough emblems! The minimum amount is {}", minAmount);
                return OnGossipSelect(player, creature, sender, ACTION_CLOSE);
            }
    ...
    bool OnGossipSelectCode(Player* player, Creature* creature, uint32 sender, uint32 action, const char* code)
    {
        ...
        case GOSSIP_SENDER_TRANSFER_JUSTICE:
            emblemId = ITEM_BADGE_OF_JUSTICE;
            break;

        case GOSSIP_SENDER_TRANSFER_GLORY:
            emblemId = ITEM_BADGE_OF_GLORY;
            break;

        case GOSSIP_SENDER_TRANSFER_COURAGE:
            emblemId = ITEM_BADGE_OF_COURAGE;
            break;
        }
