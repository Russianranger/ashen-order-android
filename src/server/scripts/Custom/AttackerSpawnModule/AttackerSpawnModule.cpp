...
void AttackerSpawnModule::SpawnAttacker(Player* player) {
    // Check if the player is on a taxi or has recently logged in
    
    Map* map = player->GetMap();
    if (!map || map->IsBattleground() || map->IsDungeon() || map->IsRaid() || !HasRequiredItem(player) || player->IsFlying() || player->IsInFlight() || !player->IsAlive()) {
        return;
    }

    const uint32 COOLDOWN_AURA_SPELL_ID_1 = 842053;
    const uint32 COOLDOWN_AURA_SPELL_ID_2 = 80094;

    // Check if either of the cooldowns is active
    if (player->HasAura(COOLDOWN_AURA_SPELL_ID_1) || player->HasAura(COOLDOWN_AURA_SPELL_ID_2)) {
        // One of the cooldowns is active, do not spawn mobs
        return;
    }
    ...
    if (creatureEntries.empty()) {
        ChatHandler(player->GetSession()).SendNotification("No creatures to spawn for your level.");
        return;
    }
    ...
    if (player->IsMounted() && !player->IsFlying()) {
        ...
        ChatHandler(player->GetSession()).SendNotification("You have stunned!");
    }
    ...
    if (player->GetVehicle() == nullptr) {
        ...
        if (isExtra) {
            if (extraCreatureEntries.empty()) {
                ChatHandler(player->GetSession()).SendNotification("No extra creatures to spawn for your level.");
                return;
            }
        ...
        player->CastSpell(player, COOLDOWN_AURA_SPELL_ID_1, true);
    }
}
