...
static void RemovePlayersFromGroup(Player* player)
{
    ...
    if (!lowLevelGroup && !highLevelGroup)
    {
        ...
        ChatHandler(player->GetSession()).SendNotification("You were removed because your group must be either all level 60 and below, or all 61 and above, to enter Karazhan.");
    }
};
...
class kara_item_removal : public PlayerScript
{
public:
    kara_item_removal() : PlayerScript("kara_item_removal") { }

    void OnLootItem(Player* player, Item* item, uint32 count, ObjectGuid /*guid*/) override
    {
        if (player->GetMapId() != 532 || player->getLevel() > 60)
            return;
        ...
        if (itemId < 20000 || itemId > 39000)
            return;

        player->m_scheduler.Schedule(Seconds(1), [player, itemId](TaskContext /*context*/)
            {
                if (Item* item = player->GetItemByEntry(itemId))
                {
                    player->DestroyItem(item->GetBagSlot(), item->GetSlot(), true);
                    ChatHandler(player->GetSession()).PSendSysMessage("An item has mysteriously vanished from your inventory...");
                }
            });
    }
};
