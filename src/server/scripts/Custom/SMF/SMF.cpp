...
class ravager_creature_script : public CreatureScript
{
public:
    ravager_creature_script() : CreatureScript("ravager_creature_script") { }

    struct ravager_creature_scriptAI : public ScriptedAI
    {
        ravager_creature_scriptAI(Creature* creature) : ScriptedAI(creature) { }

        void Reset() override
        {
            me->CastSpell(me, 46924, true);
        }
    };

    CreatureAI* GetAI(Creature* creature) const override
    {
        return new ravager_creature_scriptAI(creature);
    }
};
